...
\begin{equation} \label{intermetallics:solutionRK}
    ^{xs}G = x_{Pd}x_{Zn}\sum_{k=0}{^kL_{Pd,Zn}}(x_{Pd}-x_{Zn})^k
\end{equation}

where $^kL_{Pd,Zn}$ is the $k$th interaction term between Pd and Zn, modeled as:

\begin{equation} \label{intermetallics:solutionL}
    ^kL_{Pd,Zn} = A+BT+CT\ln T
\end{equation}

where $A$, $B$, and $C$ are model parameters to be evaluated.
...
\begin{equation} \label{intermetallics:stoiG}
    G^{{Pd}_e{Zn}_f}=e{^o}G_{Pd}^{fcc}+f{^o}G_{Zn}^{hcp}+D+ET
\end{equation}

where ${^o}G_{Pd}^{fcc}$ and ${^o}G_{Zn}^{hcp}$ are Gibbs energies of $FCC-Pd$ and $HCP-Zn$, respectively, and D and E are parameters to be evaluated.
